...
1. Baca dulu (anti konteks penuh): `memory/CONTINUE.md` → `memory/EDGE_HUNT.md` →
   `memory/SESSION_HANDOFF.md` → `PLAN_OPERASIONAL.md` → `CHECKLIST_HARIAN.md`.
2. Pemilik mengikuti **survival + Jalan A** — KPI proses/risk, **bukan** profit tiap hari.
3. Paper **dry** di `192.168.1.107`: risk lock (loss 5%, trades 30, pos 5, lev 5);
   `agent_manager_mode` **OFF** (audit 2026-07-20: agent_flat −EV / premature);
   `agent_ab_shadow` ON; full-auto/tool-loop/autonomous OFF.
4. **Jangan** usulkan: buka H30/L2 untuk cuan, 5 hipotesis L2 paralel, varian OHLCV/H24–H32 mati,
   longgarkan risk karena 1 hari merah, live scale tanpa bukti, klaim edge tanpa PROMOTE_PAPER.
