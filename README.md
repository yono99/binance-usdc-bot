...
| Kuota Gemini per-siklus | Budget keputusan/siklus DINAMIS = `min(ceil(simbol/cycles), cap)`, `cycles = gemini_decide_seconds // poll_seconds` — pas agar SEMUA simbol dapat giliran sekali per decide-interval, tapi tak melampaui cap (jaga wall-clock: `cap × ~2dtk < poll` → SL/TP tak tertunda). Self-tuning: universe/poll berubah → budget ikut. Timeout HTTP/panggilan (`GEMINI_TIMEOUT_S`, 20s) supaya satu call hang tak membekukan siklus | `config.yaml: gemini.gemini_decide_cap` (default 24) |
