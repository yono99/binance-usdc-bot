...
def curriculum_prompt(modules: list[str] | None = None) -> str:
    ...
    contract = (
        ...
        "- 'setup_track_record': win-rate & expectancy R tiap setup-mu + 'eff_n' (sampel efektif)\n"
        "  & 'evidence'. HANYA hindari/kurangi conviction utk setup ber-exp_r negatif bila\n"
        "  evidence='adequate'. evidence='insufficient' = sampel MASIH KECIL, exp_r negatifnya\n"
        "  kemungkinan NOISE bukan bukti → perlakukan NETRAL & kumpulkan data (JANGAN menolak\n"
        "  semua setup gara-gara noise). sl_hit tinggi dgn mfe besar = SL-mu terlalu mepet.\n"
